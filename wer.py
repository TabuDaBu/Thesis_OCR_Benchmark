...
class wer:
    ...
    output_excel: Path = PROJECT_ROOT / "dataset_to_process/wer_results.xlsx"
    _wer_metric = evaluate.load("wer")

    @classmethod
    def calculate_wer(cls, ground_truth: str, prediction: str) -> float:
    ...
    def evaluate_folder(
        ...
        for sample_id, ocred_path, gt_path in self._iter_test_pairs(folder_path):
            ...
            rows.append(
                {
                    "sample": sample_id,
                    "wer": self.calculate_wer(gt_text, ocr_text),
                    "ocr_text": ocr_text,
